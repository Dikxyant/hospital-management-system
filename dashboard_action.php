...
if($action=='showmalebed'){
    $sql = "select count(AID) as count from tblward where Type='Male' and Availability=0";        
    $result=mysqli_query($connect,$sql) or die("SQL a Query");
    $out="0";
    if(mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_array($result);
...
if($action=='showfemalebed'){
    $sql = "select count(AID) as count from tblward where Type='Female' and Availability=0";        
    $result=mysqli_query($connect,$sql) or die("SQL a Query");
    $out="0";
    if(mysqli_num_rows($result) > 0){
        $row = mysqli_fetch_array($result);
